/**
 * Returns abjad value of some text.
 * @customfunction
 * @param metin Text to find abjad value.
 * @param tablo Table value must be between 0-35.
 * @param shadda Add letters under shadda given times.
 * @param detail If it is 1 returns details.
 * @returns Abjad value of given text.
 */
function abjad(metin: string | undefined, tablo: number | undefined, shadda: number | undefined, detail: number | undefined): string | number {
    let sm: number = 0;
    let s: number = 0;
    let n: number = 0;
    let shaddie: number = 0;
    let err: number = 0;
    let space: number = 0;
    let hrk: number = 0;
    let nitem: string = "";
    ...
    if (metin !== undefined) {
        if (tablo !== undefined) {
            for (let counter: number = 0; counter < metin.length; counter += 1) {
...
/**
 * Returns abjad value of some text word by word.
 * @customfunction
 * @param klmmetin Text to find abjad value.
 * @param tablow Table value must be between 0-35.
 * @param shaddaw Add letters under shadda given times.
 * @param detailw If it is 1 returns details.
 * @returns Abjad value of given text word by word.
 */
function wordbyword(klmmetin: string | undefined, tablow: any, shaddaw: any, detailw: any): string {
    if (klmmetin !== undefined) {
        klmmetin += " ";
        let calculation: string = "";
        let content: string = "";
        let word: string = "";
        let klmchoosen: string;

        for (let counter = 0; counter < klmmetin.length; counter++) {
...
/**
 * Does 'Usul-ul bast' operation.
 * @customfunction
 * @param metin Text or number.
 * @param mt Repetition amount of operation.
 * @param tablo 0 to 35 abjad table codes -1 to -36 mirror codes.
 * @param shadda Add letters under shadda given times.
 * @param language "ARABIC", "HEBREW" or "TURKISH".
 * @param detail If it is 1 returns details.
 * @returns Usul-ul bast operation results.
 */
function bastet(metin: string | number, mt: number, tablo: number, shadda: number, language: string, detail: number): string | number {
    let err: number = 0;
    let baster: number;
    let invertablo: number;
    let hm: number;
    let ns: string;

    if (typeof metin === "number" || !isNaN(parseFloat(metin as string))) {
        baster = parseFloat(metin as string);
    } else {
        switch (tablo) {
...
/**
 * Counts or lists elemental letters dependent to a guide.
 * @customfunction
 * @param metin Source text.
 * @param otabiat 0 for amount, 1 for list.
 * @param otype 0 for fire, 1 for air, 2 for water, 3 for earth.
 * @param shadda Add letters under shadda given times.
 * @param guide 0 for Turkish, 1 for Ibn-i Arabi, 2 for Al Buni, 3 for Al Husayni, 4 for Hebrew and else for commenly used guide.
 * @returns Amount or list of letters.
 */
function unsur(metin: string | undefined, otabiat: any, otype: any, shadda: number, guide: any): void {
    let counter: number, adet: number, choosen: string, liste: string, selected: string;
    selected = "";
    liste = "";
    adet = 0;
    if (metin !== undefined) {
 ...
 * @customfunction
 * @param teksirmetin Source text.
 * @param texsirayrac Seperator between letters.
 * @param shadda Add letters under shadda given times.
 * @returns Variants of text.
 */
function teksir(teksirmetin: string, teksirayrac: number, shadda: number): string {
    let iksir: string, result: string, inversed: number, newmetin: string, lengthdouble: number, produce: number, counter: number, teksired: string;
    lengthdouble = 0;
    if (shadda === 2) {
...
/**
 * Converts arabic numbers to indian.
 * @customfunction
 * @param indians Number.
 * @returns Indian number equivalants of number.
 */
function numbers2arab(indians: number | string): string {
    let na: string = "";
    indians = indians.toString();
    for (let counter: number = 0; counter < indians.length; counter++) {
        const choosenarab: string = indians[counter];
        switch (choosenarab) {
            case "1":
                na += "١";
                break;
...
/**
 * Converts number to spritual name of an entity.
 * @customfunction
 * @param num Number to read as entity name.
 * @param htype "ULVI" for high entity name, "SUFLI" for low entity name, "ŞER" for evil entity name, another arabic phrase can be used as suffix here.
 * @param method Sagheer and Kabeer abjad table codes.
 * @returns Spritual entity name produced from number.
 */
function huddam(num: number, htype: string, method: number): string {
    let suffix: number, preffix: string, h: string, gh: string = "", counts: number, departs: number, rest: number, counter: number, counting: number, choosenduty: number, turn: number, counted: number, mode: string, eacher: string;
    const hpart: string[] = [];
    hpart[0] = "nothing";
...
/**
 * Adds digits of number until enough digits left.
 * @customfunction
 * @param urval Number.
 * @param d1g1tamount Amount of digits; if 0 every sum will be listed.
 * @returns Sum of numbers digits.
 */
function rakamtopla(urval: number, d1g1tamount: number): string {
    let choosen: number, newsum: number, hepsi: string, myval: string, counter: number, showvar: string;
    myval = "" + urval + "";
    if (d1g1tamount === 0) {
        hepsi = myval;
        do {
...
/**
 * Rosary usage advisor.
 * @customfunction
 * @param zkr Number to reach.
 * @param minimum Numbers under will be multiplied by self.
 * @param boncuk Total number of rosary beads.
 * @param bolum Number of beads in one section of Rosary.
 * @returns Rosary usage advise.
 */
function tesbeh(zkr: number, minimum: number, boncuk: number, bolum: number): string {
    let turn: number, part: number, rest: number;
    let outp: string = "";
    zkr = parseFloat(zkr.toString());
    if (zkr < minimum) {
        zkr = zkr * zkr;
    }
...
function altayaz(girdi: string): string {
    let ss: string = "";
    let choosenalta: string;

    girdi = girdi.toString();
    for (let counter = 0; counter < girdi.length; counter++) {
...
/**
 * Removes characters from string which abjad function does not calculate.
 * @customfunction
 * @param metinsaf Source text.
 * @param ayrac Seperator between letters If 0 no seperator, "" is possible to use.
 * @param shadda Add letters under shadda given times.
 * @returns Purified text for abjad() function.
 */
function saf(metinsaf: string | undefined, ayrac: number, shadda: number): string {
    let safmetin: string = "";
    let irun: string;
    let choosen: string;
    let counter: number;
...
function asgar(harf: string, level: number): number {
    const c: number = parseFloat(abjad(harf, level, 1));
    if (c > 12) {
 ...
 * Adds new lines.
 * @customfunction
 * @param amount Amount of newlines.
 * @returns New lines.
 */
function newline(amount: number): string {
    let nl: string = "";
 ...
 * Reads numbers.
 * @customfunction
 * @param MyNumber Number to read.
 * @param language "ARABIC", "HEBREW", "TURKCE".
 * @returns Pronounciation of number.
 */
function nutket(MyNumber: number = 0, language: string = "ARABIC"): string {
    let temp: string = "";
    let nutuk: string = "";
...
function GetHundreds(MyNumber: number, language: string, counter: number, nutuk: string): string {
    let result: string = "";
    MyNumber = parseInt(("000" + MyNumber).slice(-3));
    const MyNumberStr: string = MyNumber.toString();
...
function GetTens(TensText: string, language: string, counter: number, Hundreds: number, nutuk: string): string {
    let result: string = "";
    let PlaceOnes: string[] = [];
    let PlaceTwos: string[] = [];
...
function GetTensStatus(Tens: number, language: string): string {
    let result: string = "";
    switch (language) {
        case "ARABIC":
            switch (Tens) {
...
function GetDigit(Digit: number, language: string): string {
    let result: string = "";
    switch (language) {
        case "ARABIC":
            switch (Digit) {
...
function Left(str: string, n: number): string {
    if (n <= 0)
        return "";
...
function Right(str: string, n: number): string {
    if (n <= 0)
        return "";
    else if (n > str.length)
        return str;
...
function hepart(npotent: number, mimic: number): number {
    let kat = 2;
    let result: number;
    do {
        result = npotent * kat;
